...
export default function PhoneInput({
  ...
  const formatPhoneForDisplay = (digits: string): string => {
    ...
    // При фокусе поле подставляет «+7», а человек нередко набирает номер
    // целиком — получается двенадцать цифр. Обрезка до одиннадцати оставляла
    // от них чужой номер: «+7» + «+77011234567» превращалось в 77701123456,
    // «+7» + «87011234567» — в 78701123456. Лишнюю ведущую убираем здесь,
    // до остальных правил, чтобы «восьмёрка → код страны» отработала как надо.
    //
    // Двенадцатая цифра, набранная сверх готового номера, тоже съест ведущую —
    // случай редкий, и поле сразу показывает результат, тогда как молча
    // подставленный не тот номер человек не замечает вовсе.
    if (clean.length === 12 && clean[0] === "7" && (clean[1] === "7" || clean[1] === "8")) {
      clean = clean.slice(1);
    }
